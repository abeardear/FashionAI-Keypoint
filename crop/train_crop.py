...
from dataset_crop2 import fashionDataset,part_name
from evaluate_crop import *
...
train_dataset = fashionDataset(annoRoot+'train_round2.csv',annoRoot+'train-argmax-cpnv2.csv',trainDataRoot,train=True,transform=[transforms.ToTensor()], \
...
test_dataset = fashionDataset(annoRoot+'valid.csv',annoRoot+'valid-argmax-cpnv2.csv',trainDataRoot,train=False,transform=[transforms.ToTensor()], \
...
logfile = open('argmax-crop13-'+select_part+'.txt','w')
def train(epoch):
    print('\nEpoch: %d' % epoch)
    net.train()
    train_loss = 0
...
def test(epoch):
    ...
    acc_average = 0
    NE_average = 0
    pred_csv(net,select_part,name='argmax-crop13') #get pred.csv
    ...
    NE_average = normalized_error(select_part,'argmax-crop13') #get NE from pred.csv and valid.csv
    ...
    if best_NE > NE_average:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state,'./checkpoint/argmax-crop13-' +select_part +'.pth')
        best_NE = NE_average
...
#     return optimizer
timefile = open('cpn-hg-v2-time.txt','w')    
test(start_epoch)
